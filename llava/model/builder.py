...
from transformers import AutoTokenizer, AutoModelForCausalLM, AutoConfig, BitsAndBytesConfig, PreTrainedTokenizer, PreTrainedModel
from typing import Dict
import torch
from llava.model import *
from llava.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, SPECIAL_FRAME_TOKEN
...
def load_pretrained_model(model_path, model_base, model_name, 
                          load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            # if "post" in model_path.lower():
            #     rank0_print("Loading stage one model...")
            #     from llava.model.language_model.llava_qwen import LlavaQwenConfig
            #     s1_pretrained = "/path/to/your/model"  # Replace with your pretrained path
            #     s1_model_base = "/path/to/your/model_base"  # Replace with your model base path
            #     s1_lora_cfg_pretrained = LlavaQwenConfig.from_pretrained(s1_pretrained)
            #     tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False)
            #     model = LlavaQwenForCausalLM.from_pretrained(s1_model_base, low_cpu_mem_usage=True, config=s1_lora_cfg_pretrained, attn_implementation=attn_implementation, **kwargs)
            ...
            #     if os.path.exists(os.path.join(s1_pretrained, "non_lora_trainables.bin")):
            #         non_lora_trainables = torch.load(os.path.join(s1_pretrained, "non_lora_trainables.bin"), map_location="cpu")
            #     else:
            ...
            #         non_lora_trainables = load_from_hf(s1_pretrained, "non_lora_trainables.bin")
            ...
            #     model = PeftModel.from_pretrained(model, s1_pretrained)
    ...
    if "llava" in model_name.lower() or is_multimodal:
        # insert the image start and end special token
