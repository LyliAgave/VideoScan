...
class ModelArguments:
    ...
    stream_post_model: Optional[bool] = field(default=False)
    train_w_vit: Optional[bool] = field(default=False)
    use_carrier: Optional[bool] = field(default=False)
    cls_only: Optional[bool] = field(default=False)
...
class DataArguments:
    ...
    stream_post_data: Optional[bool] = field(default=False)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    stream_post_train: Optional[bool] = field(default=False)
...
def find_all_linear_names(model):
    ...
    train_w_vit = getattr(model.config, "train_w_vit", False)
...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    _, im_end, _, im_start  = tokenizer.additional_special_tokens_ids 
    # tokenizer.additional_special_tokens_ids = [151647, 151645, 151643, 151644]
    # tokenizer.additional_special_tokens = ['<|frame_flag|>', '<|im_end|>', '<|endoftext|>', '<|im_start|>']
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                if self.data_args.stream_post_data:
                    sample = self._get_stream_item(i)
                else:
    ...
    def _get_stream_item(self, i) -> Dict[str, torch.Tensor]:
        # 1. get video paths in batch
        # source = {'id': '', 'conversation': '', 'data_source': '', 'video', ''}
        sources = self.list_data_dict[i]
        # if batch size = 1, add a dim of batch size
        if isinstance(i, int):
            sources = [sources]
        assert len(sources) == 1, "Don't know why it is wrapped to a list"  # FIXME
        ...
        if "video" in sources[0]:
            ...
            if not os.path.exists(video_file):
                print("File {} not exist!".format(video_file))
            # 2. get stream frames and time stamps (list)
            try:
                if "shareVideoGPTV" in video_file:
                    frame_files = [os.path.join(video_file, f) for f in os.listdir(video_file) if os.path.isfile(os.path.join(video_file, f))]
                    frame_files.sort()  # Ensure the frames are sorted if they are named sequentially
                ...
                if self.data_args.add_time_instruction:
                    ...
                    sources[0]["conversations"][0]["value"] = f'{DEFAULT_IMAGE_TOKEN}\n{time_instruciton}\n{sources[0]["conversations"][0]["value"].replace(DEFAULT_IMAGE_TOKEN, "")}'
                image = [(image, video[0].size, "video")]
                # 4. get non_im_text input_ids and label with shape [batchsize, token]
                sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)
                # print(sources)
            except Exception as e:
                print(f"Error: {e}")
    ...
    if model_args.model_class_name is not None:
        ...
        if "mixtral" in model_args.model_name_or_path.lower():
            ...
            if "moe" in model_args.model_name_or_path.lower() or "A14B" in model_args.model_name_or_path:
                ...
                if "checkpoint" in model_args.model_name_or_path.lower():
                    rank0_print("Loading LLaVA Video from checkpoint...")
                    model_base = model_args.model_name_or_path+"/checkpoint"
                    
                    from llava.model.language_model.llava_qwen import LlavaQwenConfig
                    lora_cfg_pretrained = LlavaQwenConfig.from_pretrained(model_args.model_name_or_path)
                    customized_kwargs["config"] = lora_cfg_pretrained
                    model = LlavaQwenForCausalLM.from_pretrained(model_base, 
                                                                cache_dir=training_args.cache_dir,
                                                                attn_implementation=training_args.attn_implementation,
                                                                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
                                                                low_cpu_mem_usage=False,
                    ...
                    if os.path.exists(os.path.join(model_args.model_name_or_path, "non_lora_trainables.bin")):
                        ...
                        raise ValueError(f"non_lora_trainable.bin not exist in {model_args.model_name_or_path}")
...
def train(attn_implementation=None):
    ...
    if "mistral" in model_args.model_name_or_path.lower() or "mixtral" in model_args.model_name_or_path.lower() or "zephyr" in model_args.model_name_or_path.lower():
        ...
        if "checkpoint" in model_args.model_name_or_path.lower():
            tokenizer = transformers.AutoTokenizer.from_pretrained(model_args.model_name_or_path+"/checkpoint", 
                                                                   cache_dir=training_args.cache_dir, 
    ...
    if training_args.lora_enable:
        rank0_print("***using lora peft model")
        from peft import LoraConfig, get_peft_model

        model.config.train_w_vit = model_args.train_w_vit
        ...
        rank0_print("Only model backbone will be tuned with LoRA...")
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.train_w_vit = model_args.train_w_vit
        model.config.stream_post_model = model_args.stream_post_model
        model.config.use_carrier = model_args.use_carrier
        model.config.cls_only = model_args.cls_only
