...
class LLaVATrainer(Trainer):
    ...
    #     """
    #     # common forward
    #     if self.args.stream_post_train:
    ...
    #     # previous frames do not need loss
    #     """
    #     inputs: 
    #     'input_ids': only text part
    #     'labels': same shape with input_ids
    #     'attention_mask': [bool]
    #     'image_sizes': [int]
    #     'modalities': ['video']
    #     'images': [tensor[3,3,384,384]]
    #     """
    #     # lyli: if stream post train, forward for each frame
    #     # model.eval()
    #     if self.args.stream_post_train:
    #         # lyli: if input is get from torch.no_grad(), it can't execute backward with auto gradient
    #         inputs = self._prepare_inputs(inputs)
    #         # use batch video and query to init a video streamer
    #         vstream = VideoStreamer(None, None, stream_post_train=self.args.stream_post_train, **inputs)
    #         # for each frame t-1 one forward to prefill
    #         if self.args.lora_enable:
    #             inputs = vstream.prepare_stream_train_inputs(model.module.base_model.model, self.tokenizer)
    #         else:
    #             inputs = vstream.prepare_stream_train_inputs(model.module, self.tokenizer)
    #         print(f"**********{inputs['inputs_embeds'].shape}**********")
    #     # lyli: gather to same device
    #     inputs = self._prepare_inputs(inputs)

    #     model.train()
    #     # lyli: set auto grad
    #     # for k, v in inputs.items():
    #     #     if k in 'inputs_embeds':
    #     #         inputs[k].requires_grad_()
    #     #     elif k in 'past_key_values':
    #     #         for l in range(len(v[0])):
    #     #             for i in range(len(v[0])):
    #     #                 inputs[k][l][i].requires_grad_()

    #     with self.compute_loss_context_manager():
    #         loss = self.compute_loss(model, inputs) # lyli: 在这改！
    ...
    def create_accelerator_and_postprocess(self):
        ...
        # lyli: remove the dispatch arguement and transfer it by DataLoader
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        # if self.args.lora_enable:
        super(LLaVATrainer, self)._save_checkpoint(model, trial, metrics) # lyli: fix to save the train states
    ...
    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        ...
        # super(LLaVATrainer, self)._save(output_dir, state_dict) # lyli: fix to save model train states
