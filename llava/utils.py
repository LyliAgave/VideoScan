...
def process_video_with_decord(video_file, data_args):
    ...
    return:
    video: List
    video_time: total video last time
    frame_time: time instruction for this video
    num_frames_to_sample: frame num
    """
...
# lyli: load video
def process_stream_video_with_decord(video_file, data_args):
    """
    return in stream -> list[num_frame]
    video_frames: list[num_frame]
    frame_times: list[time stamp for each frame]
    total_video_time: video last time
    avg_fps: 
    """
    if data_args.frames_upbound == 0:
    ...
    frame_timestamp = [i/fps for i in frame_idx]

    if len(frame_idx) > data_args.frames_upbound or data_args.force_sample:
        sample_fps = data_args.frames_upbound
        uniform_sampled_frames = np.linspace(0, total_frame_num - 1, sample_fps, dtype=int)
        frame_idx = uniform_sampled_frames.tolist()
        frame_timestamp = [i/avg_fps for i in frame_idx]
    ...
    return video_frames, total_video_time, frame_timestamp, num_frames
